...
subtitle: "Looking Into New York City's Airbnb Listings"
...
It has four main panes. This one — assuming you're reading this on RStudio  — is the "code editor" (also known as "script editor" and "source editor"). This is where we'll spend most of our time. I'll explain the other panes later. 
...
Let's take a look at the data. Open the Excel file. Browse around a bit.

*: Can you explain what each column is about?*

🤔 Are there any data that you'd like to have but is not there?
...
Data analysis happens within a broader context. Usually, there's an overarching business, policy, or scientific question that one would like to answer. Often that question is not very clear. Whatever the case, you need to approach the data with curiosity about the larger context.  
...
The symbol `#` at the start of a line means the line is a comment. These lines are left alone (that is, not executed) when you run a code chunk. You can use comments either to explain your code or to temporarily prevent a line of code from executing. If, below, the lines containing `install.packages` are "commented out," please remove the `#`s before running the code chunk.
...
```{r}
df <- read_csv("airbnb_nyc_data.csv")
```
...
🤔 Why is "34265" of character type? What's a real-world example of this kind of data?
...
🏏 You'll see there's another data file in the folder: "us_babynames.csv". Load the data from this file to a dataframe (hint: `read_csv`). Assign the dataframe to a variable named `df_babynames`. 
...
## The perils of the wrong data type
...
But, first, let's learn how to reference a column in a dataframe: first the name of the dataframe, then `$` and then the column name. For example: `df$cancellation_policy` or `df_babynames$Gender`.

todo-clearly:: To change data type, we apply the relevant function, such `as.character` or `as.factor`, to a column and then assign the output of the function (values with changed data type) to the same column. 
...
Some values will almost inevitably be missing in a medium to large dataset. Let's inspect our data for missing values using the function `summary`, which will give us plenty of other relevant information as well.
...
Example: Return aataframe with only those rows where the neighborhood is Chelsea.
...
[Swirl](https://swirlstats.com) is a unique tool – it teaches you R and data science interactively right on the console. Follow the instructions [here](https://swirlstats.com/students.html).
