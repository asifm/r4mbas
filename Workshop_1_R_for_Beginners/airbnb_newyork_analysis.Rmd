...
Side note: The last function `mdy` is different from the others. Normally, working with date-time in R (and in prgramming in general) is not simple. The lubridate library, which we loaded earlier, makes the job much easier. We'll not go into the details of date-times. What we did here with `mdy` is convert character data into date data. We used `mdy` because the character data were in the form month-date-year. If they were in the form ,for example, year-month-date, we would've used the function `ymd`.
